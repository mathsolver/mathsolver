...
it('it can differentiate roots', function () {
    $tree = StringToTreeConverter::run('deriv(root(x, 2))');
    $result = RootRule::run($tree);
    expect($result)->toEqual(StringToTreeConverter::run('frac(1, 2) * root(x, 2)^-1'));
});

it('can differentiate cube roots', function () {
    $tree = StringToTreeConverter::run('deriv(root(x, 3))');
    $result = RootRule::run($tree);
    expect($result)->toEqual(StringToTreeConverter::run('frac(1, 3) * root(x, 3)^-2'));
});
